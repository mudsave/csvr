...
public class SetAssetBundleName : Editor
{
    ...
    public static Dictionary<string, System.Action<string>> setPathDirAllName = new Dictionary<string, System.Action<string>>();

    [@MenuItem("EditTools/SetAssetBundleName（设置AssetBundleName）")]
    static public void start()
    {
        setPathDirAllName.Add("/config", SetAssetBundleNameByDirPathAll);
    ...
    static void SetAssetBundleNameByDirPath(string source)
    {
        ...
        {
            Debug.LogError("Enter Config");
            setPathDirAllName["/config"](source);
            return;
        }
    ...
    static void SetAssetBundleNameByDirPathAll(string source)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if (!files[i].Name.EndsWith(".meta"))
            {
                ...
                string path = GetRelativePath(files[i].FullName);
                Debug.LogError(path);
                AssetImporter depobj = AssetImporter.GetAtPath(path);
                if (depobj == null)
                {
                    Debug.LogError(null);
                }
                string bundleName = "Assets\\Resources_\\config\\configs" + ".unity3d";
                depobj.assetBundleName = bundleName;
            }
        }
    }
