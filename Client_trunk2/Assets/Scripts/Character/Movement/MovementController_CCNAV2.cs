...
/// 使用CharacterControll.Move()组件的移动，便是使用Nav Mesh Agent来获取移动路点
/// @TODO(PHW)：这是一个未完成的版本，当在高处向低处执行StartMoveForward()时，会发现角色并没有贴着地面移动，而是悬在半空中
/// </summary>
[AddComponentMenu("Character/MovementController/CC + NAV")]
[RequireComponent (typeof(CharacterController))]
public class MovementController_CCNAV2 : MovementController {
	/// <summary>
	/// 推力，力越大推动的速度越快
	/// 小于等于0时则无法推动任何人
	/// </summary>
	...
	*/
	...
	IEnumerator _MovingMonitor()
	{
		...
		while (m_movingParam.moving)
		{
			...
			// 移到指定的位置
			var moveTo = (m_currentMoveTo - m_myTransform.position).normalized * m_movingParam.speed * Time.deltaTime;
