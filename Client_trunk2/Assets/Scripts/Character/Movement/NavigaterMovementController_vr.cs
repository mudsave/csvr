...
public class NavigaterMovementController_vr : MovementController
{
    Animator m_animator;
    ...
    IEnumerator _MoveForwardMonitor()
    {
        ...
        while (m_movingForwardParam.moving)
        {
            ...
            Vector3 v = m_myTransform.forward * offsetDirection.z + m_myTransform.right * offsetDirection.x;
            //m_navMeshAgent.speed =m_movingForwardParam.speed * v.magnitude;
            m_navMeshAgent.speed = m_movingForwardParam.speed;
            //Debug.Log("V:" + v.magnitude + ";speed:" + m_navMeshAgent.speed);
            v.Normalize();
            m_navMeshAgent.SetDestination(m_myTransform.position + v);
            if (m_isMoveByFinger) // 只有玩家控制角色移动的时候做碰撞检测
            {
                if (m_capCollide)
                {
    ...
    {
        StartMoveForward(Vector3.forward);
    }

    protected override void StartMoveForward(Vector3 offsetDirection)
    {
        this.offsetDirection = offsetDirection;
        m_navMeshAgent.Resume();
        m_navMeshAgent.stoppingDistance = 0.1f;
        m_navMeshAgent.speed = m_movingForwardParam.speed;
        //m_navMeshAgent.areaMask = KBEngine.World.instance.m_areaMask; //<todo:yelei>临时写法
