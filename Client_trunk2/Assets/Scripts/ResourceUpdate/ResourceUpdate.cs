...
public class ResourceUpdate : MonoBehaviour {
...
#else
    public static readonly string SERVER_IP_URL = "http://127.0.0.1/";
    ...
    public void LoadRes(ResFinishDownload func)
    {
        StartCoroutine(DownLoad(SERVER_RES_URL + VERSION_FILE, delegate(WWW www)
        {
            ...
            if (File.Exists(LOCAL_RES_PATH + "/AssetBundles/" + VERSION_FILE))
            {
                localResXMl.Load(LOCAL_RES_PATH + "/AssetBundles/" + VERSION_FILE);
    ...
    IEnumerator LoadResFromServerVersion(ResFinishDownload func)
    {
        ...
        {
            Object obj = Resources.Load(CPrefabPaths.UpdateMessage);
            GameObject updateWin = Instantiate(obj) as GameObject;
        }
        int downLoaded = 0;
        ...
        while(enumerator.MoveNext())
        {
            ...
            yield return StartCoroutine(DownLoad((SERVER_IP_URL + element).Replace(" ","%20").Replace("#","%23"), delegate(WWW www)
            {
                ReplaceLocalRes(element, www.bytes);
                GlobalEvent.fire("ShowDownLoadProgressInfo", new object[] { ++downLoaded, downloadFileList.Count });
            }));
        }
