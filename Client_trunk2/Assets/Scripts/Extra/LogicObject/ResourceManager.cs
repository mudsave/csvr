...
public class ResourceManager : MonoBehaviour
{
    ...
    public static IEnumerator IncreaseLoadResouresSync(string path, bool isforerer)
    {
        ...
        AssetBundleLoadAssetOperation request = ResourceLoader.LoadAsync(path);
    ...
    //同步加载一个模型的所有资源
    public static void LoadModelRes(string modelID, bool isforerer)
    {
        if (!CConfigClass.modelConfig.ContainsKey(modelID))
            return;

        CModelConfig config = CConfigClass.modelConfig[modelID];
        if (config.path != "")
            ResourceManager.IncreaseLoadResoures(config.path, isforerer);

        if (config.controller != "")
            ResourceManager.IncreaseLoadResoures(config.controller, isforerer);

        if (config.showController != "")
            ResourceManager.IncreaseLoadResoures(config.showController, isforerer);
    }
    ...
    /// <param name="isforerer"></param> 
    public static IEnumerator LoadModelResSync(string modelID, bool isforerer)
    {
        if (!CConfigClass.modelConfig.ContainsKey(modelID))
            yield break;
        ...
        if (config.path != "")
            yield return CGameObject.instance.StartCoroutine(IncreaseLoadResouresSync(config.path, isforerer));

        if (config.controller != "")
            yield return CGameObject.instance.StartCoroutine(IncreaseLoadResouresSync(config.controller, isforerer));

        if (config.showController != "")
            yield return CGameObject.instance.StartCoroutine(IncreaseLoadResouresSync(config.showController, isforerer));
