...
def onBaseAppReady(isBootstrap):
	...
	if isBootstrap:
		# 初始化地图
		global g_spaceManager
		g_spaceManager = KBEngine.createBaseLocally( "SpaceManager" )
		g_spaceManager.init()
		
	# 在baseApp成功起来以后创建一个全局的baseapp Entity
	# 被创建的entity会自动注册为global base entity
	global g_baseApp
	g_baseApp = KBEngine.createBaseLocally( "BaseappEntity" )
...
def onReadyForLogin(isBootstrap):
	...
	global g_spaceManager
	if g_spaceManager == None:
		return 0
		
	initProgress += g_spaceManager.initProgress
	
	return initProgress / 1
...
# init config
import Extra.BaseConfig
							
import MapDataConfig
from Extra.BaseConfig import g_mapDataConfig
